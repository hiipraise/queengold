...
export default function VerifyClient({ initialSerial = "" }: Props) {
  ...
  // Auto-verify if sn param already present (QR scan flow)
  useEffect(() => {
    const sn = searchParams.get("sn");
    if (sn) {
      setSerial(sn.toUpperCase());
      handleVerify(sn.toUpperCase());
    }
  ...
  async function handleVerify(overrideSerial?: string) {
    ...
    // Update URL without full navigation (SEO-friendly, shareable)
  ...
  return (
    <main className="relative min-h-screen flex flex-col items-center justify-start px-4 pb-16 overflow-x-hidden z-10">
      ...
      {!passport && (
        <section
          className="w-full max-w-md mx-auto mt-10 animate-fade-up delay-200"
          aria-label="Watch verification form"
        >
          <div className="card-luxury p-8 sm:p-10">
            ...
            <p className="text-center font-body text-sm tracking-widest uppercase mb-8"
            ...
            <div className="space-y-4">
              ...
              {/* Error states */}
              {notFound && (
                <p id="serial-error" className="text-sm text-center animate-fade-in"
                   style={{ color: "#E8A0A0", letterSpacing: "0.05em" }}>
      ...
      <footer className="mt-auto pt-12 text-center">
        <p className="font-body text-xs tracking-widest uppercase opacity-30"
           style={{ color: "var(--gold-muted)" }}>
          www.queengold.com
        </p>
      </footer>
  ...
  return (
    <svg width="12" height="14" viewBox="0 0 12 14" fill="none" aria-hidden="true">
      <rect x="1" y="6" width="10" height="8" rx="1.5"
            ...
            style={{ color: "var(--gold-muted)" }} />
      <path d="M3 6V4.5a3 3 0 0 1 6 0V6"
