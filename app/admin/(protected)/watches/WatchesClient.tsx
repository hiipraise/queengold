...
export default function WatchesClient() {
  ...
  // Debounce search
  useEffect(() => {
    const t = setTimeout(() => { setPage(1); fetchWatches(); }, 400);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="font-body text-xs mt-1 tracking-wide"
          ...
          <thead>
            <tr style={{ borderBottom: "1px solid var(--border-gold)" }}>
              {["Serial", "Model", "Collection", "Status", "Warranty", "Scans", "First Scan", "Date of Purchase", "Actions"].map(h => (
                <th key={h}
                    className="px-4 py-3 text-left font-display text-xs tracking-[0.15em] uppercase whitespace-nowrap"
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={w.serialNumber}
                ...
                <td className="px-4 py-3 font-body text-xs tracking-widest font-medium"
                ...
                <td className="px-4 py-3">
                  <WarrantyBadge value={w.warrantyStatus} />
                </td>
                <td className="px-4 py-3 font-body text-xs text-center"
                    style={{ color: w.scanCount > 5 ? "#E8C060" : "var(--text-muted)" }}>
                  {w.scanCount}
                  {w.scanCount > 10 && (
                    <span className="ml-1 text-[10px]" title="High scan count — verify authenticity">⚠</span>
                  )}
                </td>
      ...
      {/* ── Add / Edit Modal ────────────────────────────────────── */}
      {showModal && (
        <Modal title={editSerial ? `Edit ${editSerial}` : "Register New Watch"}
          ...
          <div className="space-y-4">
            ...
            {formError && (
              <p className="text-sm animate-fade-in" style={{ color: "#E8A0A0" }}>{formError}</p>
            )}

            <div className="flex gap-3 pt-2">
              <button onClick={handleSave} disabled={saving}
                      className="btn-gold flex-1 h-11 text-xs rounded-sm">
      ...
      <div className="card-luxury w-full max-w-lg max-h-[90vh] overflow-y-auto p-8 animate-fade-up">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={onClose}
                  className="font-body text-xl opacity-40 hover:opacity-80 transition-opacity"
